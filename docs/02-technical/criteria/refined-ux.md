...
**Adaptive layout.** `MainActivity` resolves a `WindowWidthSizeClass` and exposes it through `LocalWindowWidthSizeClass`. On `Expanded` (tablets, foldables, desktop mode) `MainScreen` swaps the collapsing layout for `ExpandedMainContent` (two static columns), `SettingsScreen` renders cards in `LazyVerticalGrid(GridCells.Fixed(2))`, and `UnifiedTransactionListScreen` puts the trend chart on the left and the transaction list on the right. The `Compact` / `Medium` phone branches are deliberately untouched.
...
**Tablet / Expanded layout — main, settings, statistics**

![Main Screen — Expanded](../../assets/images/main_screen_tablet.png)

![Settings — Expanded](../../assets/images/settings_tablet.png)

![Statistics — Expanded](../../assets/images/statistics_tablet.png)
...
| 12 | Adaptive layout for tablets / foldables | Done | `WindowWidthSizeClass` via `LocalWindowWidthSizeClass`; Expanded branches in `MainScreen`, `SettingsScreen`, `UnifiedTransactionListScreen`; tablet screenshots referenced above |
