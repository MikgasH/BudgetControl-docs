...
│   └── util/WindowSize.kt             # LocalWindowWidthSizeClass CompositionLocal
├── src/debug/                         # HiltTestActivity (ComponentActivity + @AndroidEntryPoint)
└── src/androidTest/
    ├── di/{TestDatabaseModule, TestNetworkModule}    # @TestInstallIn fakes
    └── …Test.kt                       # OnboardingLanguagePageTest, MainScreenFabNavigationTest
```

| Area | Decision |
|---|---|
| Reusable components | `PieChart`, `TransactionItem`, `AmountInputCard`, `CategorySelector`, `CurrencySelector`, `BankSelector`, `CustomColorPicker`, `DatePickerDialog`, `PeriodRangePicker`, `CategoryIconWithLimitRing`, `CategoryTrendBarChart`, `TrendChartLegend` |
...
| 6 | Responsive layout | Done | `material3-window-size-class`; `LocalWindowWidthSizeClass` in `ui/util/WindowSize.kt`; Expanded branches in `MainScreen` (`ExpandedMainContent`), `SettingsScreen` (`LazyVerticalGrid` 2 columns) and `UnifiedTransactionListScreen` (chart left + list right) |
...
| 14 | Data visualization | Done | Canvas-based charts: RateHistoryScreen (PCHIP line chart), CategoryTrendBarChart (stacked bars with limit line), CategoryIconWithLimitRing (circular progress arc) |

### Tablet / Expanded Window Layout

On tablets and foldables (`Expanded` `WindowWidthSizeClass`), `MainScreen`, `SettingsScreen` and `UnifiedTransactionListScreen` switch to two-column layouts; the `Compact`/`Medium` phone branches are kept untouched to minimise regression risk. Reference screenshots are catalogued under [Refined UX → Key Screens](refined-ux.md).
...
| Compose UI tests cover only golden paths (Onboarding language page + Main FAB navigation) | Most screens still rely on manual QA | Expand suite to cover transaction form, settings, accounts |
