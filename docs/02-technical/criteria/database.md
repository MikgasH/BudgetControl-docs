...
- **Room SQLite v17** on Android with fourteen migrations (3→17). The 23 preset banks are inserted via the Room `onCreate` / `onOpen` callback when `banks` is empty; the 24 default categories are seeded from `BudgetControlApplication.onCreate()` through `CategoryRepository.initializeDefaultCategories()` (kept outside the Room callback because it depends on injected repositories and string resources for localisation).
...
**Room (Android, v17, 9 entities):**

`ExpenseEntity`, `IncomeEntity`, `CategoryEntity`, `BankEntity`, `CurrencyExchangeEntity`, `AccountEntity`, `AccountGroupEntity`, `AccountGroupMemberEntity`, `CategoryLimitEntity`. Foreign keys cascade on category and account deletion; indexes cover `categoryId`, `date`, `accountId` (added v15→16), `type`, `isFavorite`, and `(fromCurrency, toCurrency)`. Migrations 3→17 cover column additions (`originalAmount`, `bankCommission`, `rateSource`, `accountId`), five new tables (`currency_exchanges`, `accounts`, `account_groups`, `account_group_members`, `category_limits`), and nine index additions.

`category_limits` table (v17):
- `categoryId`: TEXT (PK, FK → categories ON DELETE CASCADE)
- `amount`: REAL (limit stored in base currency)
...
**Room migrations (recent):**

| Migration | Purpose |
|---|---|
| 15 → 16 | Adds `Index("accountId")` on `expenses` and `incomes` for per-account filter pushdown |
| 16 → 17 | Creates category_limits table with FK → categories CASCADE; enables per-category monthly spending limits |

### Diagrams

**Android Room Schema (version 17)**
...
| 6 | Schema via migrations | Done | Liquibase v1.0–v1.9; Room migrations 3→17                                                                                                                                                                                                                                                                |
