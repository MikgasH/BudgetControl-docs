...
│                         # filter/PublicEndpointRateLimitFilter
├── analytics-service/    # controller / service / client (RestClient + Caffeine)
│                         # filter/TrendsEndpointRateLimitFilter
├── cerps-common/
│   ├── filter/{AbstractRateLimitFilter, RateLimitRule, RateLimitEntry}
│   └── …                 # shared DTOs, error model, constants, logback-common.xml
└── .github/workflows/ci.yml   # build + tests + JaCoCo gate 70%
```

Liquibase changelogs v1.0–v1.9 under `currency-service/resources/db/changelog/`.
...
| `AbstractRateLimitFilter` in `cerps-common` | Single contract across services; X-Forwarded-For handling and bucket logic deduplicated |
...
| 7 | Structured logging | Done | `logstash-logback-encoder` 8.0; shared `cerps-common/logback-common.xml`; per-service `logback-spring.xml` switches by Spring profile — `dev` plain text with `correlationId`, `prod` JSON via `LogstashEncoder` with `service` field; `currency.rates.refresh.failures` / `currency.provider.failures` metrics |
| 8 | Production deployment | Done | Railway over HTTPS |
| 9 | Coverage ≥ 70% | Done | 74% / 77% JaCoCo, gate enforced via `jacoco-check` execution in parent `pom.xml` and on every PR through `.github/workflows/ci.yml` |
...
|---|---|---|
| Railway deploy still triggered manually | Push-to-Railway not automated | Extend `ci.yml` with a deploy job once a staging env is set up |
