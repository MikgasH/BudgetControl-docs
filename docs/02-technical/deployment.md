...
2. `./mvnw verify` — compile, unit and integration tests for both services.
3. JaCoCo coverage report.
4. **Coverage gate `LINE` ≥ 70%** — the `jacoco-check` execution lives in the parent `pom.xml`, so the same gate runs locally via `./mvnw verify` and on CI without separate configuration.

The `mvnw` executable bit is pinned in the git index via `git update-index --chmod=+x mvnw`, which removes the recurring "permission denied" failure on Windows checkouts. Railway deploys still trigger manually after CI is green.

## Logging

Logback is configured once in `cerps-common/src/main/resources/logback-common.xml` (shared appenders, levels, `correlationId` pattern) and pulled into each service's `logback-spring.xml` with `<include resource="logback-common.xml"/>`. Spring profiles select the encoder:

- **`dev`** — plain text with a `correlationId` field for IDE-friendly local debugging.
- **`prod`** — `LogstashEncoder` (JSON) plus a custom `service` field (`currency-service` / `analytics-service`) so the Railway log stream can be parsed by an aggregator without per-service rules.

`logstash-logback-encoder` 8.0 is declared once in the root `pom.xml` and inherited by both services.
