...
- **`TrendChartData` sealed interface cleanly encapsulates three chart modes (paired/stacked/single) — no conditional logic leaks into the composable layer.**
- **Category limits integrated into existing create/edit flows rather than a separate settings screen — reduced UI surface area.**
- **CI gate caught regressions before they reached Railway.** GitHub Actions runs build + tests + a JaCoCo `LINE` ≥ 70% gate on every PR; locking the `mvnw` executable bit in the git index removed the recurring Windows-checkout failure.
- **Pulled rate-limit logic into `cerps-common`.** `AbstractRateLimitFilter` plus shared `RateLimitRule` / `RateLimitEntry` removed duplication between the Currency and Analytics services and let X-Forwarded-For be re-enabled safely behind a single helper.
- **Structured JSON logging unblocked log aggregation.** `logstash-logback-encoder` in `prod`, plain text + `correlationId` in `dev`, configured once in `cerps-common/logback-common.xml`.
- **WindowSizeClass added without regressing phone layouts.** Compact / Medium branches were left unchanged; Expanded was a separate composable in `MainScreen`, `SettingsScreen` and `UnifiedTransactionListScreen`.
- **First Compose UI integration tests landed.** `HiltTestRunner` + `HiltTestActivity` + fake `TestDatabaseModule` / `TestNetworkModule` give the suite a foundation for future golden-path coverage.

## What Did Not Work as Planned

- **Railway deploy still manual.** CI builds and gates the code, but pushing the container to Railway is still a manual step.
...
- **UI test coverage is still narrow.** Two test files / three tests is a foundation, not a safety net — most screens still rely on manual QA.
...
- Extend `ci.yml` with a SonarCloud gate, Docker image push and an automated Railway deploy job (build + JaCoCo gate already shipped in v12).
- Unified `TransactionEntity` once transfers between accounts enter scope.
- JSON export and import via `ActivityResultContracts`.
- Expand the Compose UI suite beyond the Onboarding language page and Main FAB navigation tests.
