...
- Category spending limits with circular progress ring (green/amber/red based on utilization)
- Stacked bar trend chart in transaction list (paired expense/income or per-category breakdown)
- Scroll-aware FAB in UnifiedTransactionListScreen
- Adaptive layout for tablets and foldables (`WindowWidthSizeClass` `Expanded` branches in `MainScreen`, `SettingsScreen`, `UnifiedTransactionListScreen`); phone branches unchanged
